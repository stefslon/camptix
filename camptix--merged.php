...
class CampTix_Plugin {
	 ...
	 */
	function question_field_text( $name, $value, $required = false ) {
		?>
	 ...
	 */
	function question_field_textarea( $name, $value, $required = false ) {
		?>
		...
		<div id="tix">
			...
			<?php if ( $available_tickets ) : ?>
				...
				<div class="row hide-for-small header">
						...
						<div class="large-6 columns"><h6><?php _e( 'Description', 'camptix' ); ?></h6></div>
						<div class="large-2 columns text-center"><h6><?php _e( 'Price', 'camptix' ); ?></h6></div>
						<?php if ( apply_filters( 'camptix_show_remaining_tickets', true ) ) : ?>
							<div class="large-2 columns text-center"><h6><?php _e( 'Remaining', 'camptix' ); ?></h6></div>
						<?php endif; ?>
						<div class="large-2 columns text-center <?php echo implode( ' ', apply_filters( 'camptix_quantity_row_classes', array( 'tix-column-quantity' ) ) ); ?>">
							<h6><?php _e( 'Quantity', 'camptix' ); ?></h6>
						</div>
					...
					<?php foreach ( $this->tickets as $ticket ) : ?>
						<?php
							...
							?>
							<div class="row tix-ticket-<?php echo absint( $ticket->ID ); ?>">
							...
							<div class="large-2 small-4 columns text-center"><h5><span class="show-for-small"><?php _e( 'Price', 'camptix' ); ?></span>
								<?php if ( $price > 0 ) : ?>
							...
							<div class="large-2 small-4 columns text-center <?php echo implode( ' ', apply_filters( 'camptix_quantity_row_classes', array( 'tix-column-quantity' ) ) ); ?>"><h5><span class="show-for-small"><?php _e( 'Rem.', 'camptix' ); ?></span><?php echo $ticket->tix_remaining; ?>
							<?php 
						...
						<div class="large-2 small-4 columns text-center"><h5>
						...
						<?php if ( $this->have_coupons() ) : ?>
							<div class="row">
								<div class="large-6 columns large-offset-6 text-right">
									<?php if ( $this->coupon ) : ?>
									...
									<?php else : ?>
										...
										<div id="tix-coupon-container" style="display: none;" class="row collapse">
											...
											<div class="large-4 small-4 columns">
												<input type="submit" name="tix_coupon_submit" value="<?php esc_attr_e( 'Apply Coupon', 'camptix' ); ?>" class="postfix secondary button radius" />
											</div>
										</div>
										<script>
	...
	function form_attendee_info() {
		...
		<div id="tix" class="tix-has-dynamic-receipts">
			...
			<form action="<?php echo esc_url( add_query_arg( 'tix_action', 'checkout' ), $this->get_tickets_url() ); ?>#tix" method="POST">
				...
				<h4><?php echo apply_filters( 'camptix_register_order_summary_header', __( 'Order Summary', 'camptix' ) ); ?></h4>
				<div class="row hide-for-small header">
					<div class="large-6 columns"><h6><?php _e( 'Description', 'camptix' ); ?></h6></div>
					<div class="large-2 columns text-center"><h6><?php _e( 'Per Ticket', 'camptix' ); ?></h6></div>
					<div class="large-2 columns text-center"><h6><?php _e( 'Quantity', 'camptix' ); ?></h6></div>
					<div class="large-2 columns text-center"><h6><?php _e( 'Price', 'camptix' ); ?></h6></div>
				</div>
						...
						<?php foreach ( $this->tickets_selected as $ticket_id => $count ) : ?>
							...
							<div class="row">
								...
								<div class="large-2 small-4 columns text-center"><h5><span class="show-for-small">Per Ticket</span>
								<?php if ( $price > 0 ) : ?>
									<?php echo $this->append_currency( $price ); ?>
								<?php else : ?>
									<?php _e( 'Free', 'camptix' ); ?>
								...
								<div class="large-2 small-4 columns text-center"><h5><span class="show-for-small">Qty.</span><?php echo intval( $count ); ?></h5></div>
								<div class="large-2 small-4 columns text-center"><h5><span class="show-for-small">Price.</span><?php echo $this->append_currency( $price  * intval( $count ) ); ?></h5></div>
							</div>
						<?php endforeach; ?>
						<div class="row">
							<div class="large-10 small-8 columns text-right"><p>
								<?php if ( $this->coupon ) : ?>
									<?php
										$discount_price = (float) $this->coupon->tix_discount_price;
										$discount_percent = (float) $this->coupon->tix_discount_percent;
							...
							<div class="large-2 small-4 columns text-center"><h5><strong><?php echo $this->append_currency( $total ); ?></strong></h5></div>
						</div>

				<h4 id="tix-registration-information"><?php echo apply_filters( 'camptix_register_registration_info_header', __( 'Registration Information', 'camptix' ) ); ?></h4>
				<?php foreach ( $this->tickets_selected as $ticket_id => $count ) : ?>
					<?php foreach ( range( 1, $count ) as $looping_count_times ) : ?>
						...
						<fieldset class="tix_tickets_table tix-attendee-form">
		...
		<?php do_action( 'camptix_notices' ); ?>
			...
			?>
				...
				<?php foreach ( $attendees as $attendee ) : $count++; ?>
					...
					<div class="row">
						<div class="large-6 columns">
							<h5><?php echo esc_html( $this->format_name_string( "%first% %last%", $first_name, $last_name ) ); ?></h5>
			...
			<form action="<?php echo esc_url( add_query_arg( 'tix_action', 'edit_attendee' ) ); ?>#tix" method="POST">
				...
				<div class="row">
					<div class="large-12 columns">
						...
						<input type="submit" value="<?php echo esc_attr( $submit_button_value ); ?>" class="button radius right" />
					</div>
				</div>
